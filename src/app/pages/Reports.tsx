...
const getProgramCategory = (programName: string): string => {
  ...
  if (name.includes('fitness') || name.includes('strength') || name.includes('chi kung') ||
      name.includes('walking') || name.includes('men\'s moves')) {
    return 'Healthy Living';
  }
  if (name.includes('art') || name.includes('lunch') || name.includes('digital') ||
      name.includes('mentoring')) {
    return 'Interest & Social';
  }
  if (name.includes('homework') || name.includes('playgroup') || name.includes('dungeons')) {
    return 'Young People';
  }
...
// Age groups
const AGE_GROUPS = ['under 18', '18-24', '25-44', '45-64', '65+'] as const;
...
// Councils
const COUNCILS = ['Adelaide Hills Council', 'Other Council'] as const;
...
export default function Reports() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      (participantsData || []).forEach(p => {
        ...
        // ATSI status
        if (p.identify_aboriginal_tsi) atsi.add(p.identify_aboriginal_tsi);

        // CALD background (based on country of birth or language)
        const isCald = p.country_of_birth && p.country_of_birth !== 'Australia' ||
                       p.speak_other_language === 'Yes';
        cald.add(isCald ? 'Yes' : 'No');

        // Council
        const council = p.council_region === 'Adelaide Hills Council' ? 'Adelaide Hills Council' : 'Other Council';
        couns.add(council);
  ...
  const reportData = useMemo<ReportData>(() => {
    ...
    const filteredParticipants = participants.filter(p => {
      ...
      if (filters.ageGroup !== 'all' && ageGroup !== filters.ageGroup) return false;
      if (filters.gender !== 'all' && p.gender !== filters.gender) return false;
      if (filters.atsiStatus !== 'all' && p.identify_aboriginal_tsi !== filters.atsiStatus) return false;
      if (filters.caldBackground !== 'all' && (isCald ? 'Yes' : 'No') !== filters.caldBackground) return false;
      if (filters.council !== 'all' && council !== filters.council) return false;
      if (filters.township !== 'all' && p.township !== filters.township) return false;
    ...
    const filteredPrograms = programs.filter(prog => {
      const cat = getProgramCategory(prog.name);
      if (filters.programCategory !== 'all' && cat !== filters.programCategory) return false;
      if (filters.program !== 'all' && prog.name !== filters.program) return false;
      return true;
    });
    ...
    const uniqueParticipantIds = new Set(filteredRecords.map(r => r.participant_id));
    ...
    const attendanceRate = totalRecords > 0 ? (totalAttendances / totalRecords) * 100 : 0;
    ...
    const ageDistribution = Array.from(ageDistMap.entries()).map(([name, value]) => ({ name, value }));

    // Gender distribution
    const genderDistMap = new Map<string, number>();
    filteredParticipants.forEach(p => {
      if (p.gender) {
        genderDistMap.set(p.gender, (genderDistMap.get(p.gender) || 0) + 1);
      }
    });
    const genderDistribution = Array.from(genderDistMap.entries()).map(([name, value]) => ({ name, value }));
    ...
    filteredRecords.forEach(record => {
      const prog = programs.find(p => p.id === record.program_id);
      if (!prog) return;
    ...
    const programData = Array.from(programDataMap.entries()).map(([program, data]) => ({
      program,
      category: data.category,
      uniqueParticipants: data.participantIds.size,
      attendances: data.attendances,
      attendanceRate: data.totalRecords > 0 ? (data.attendances / data.totalRecords) * 100 : 0,
    }));
  ...
  const handleExport = async (format: 'pdf' | 'csv') => {
    if (format === 'pdf') {
      ...
      // Export CSV
      const csvData = [
        ['Program', 'Category', 'Unique Participants', 'Attendances', 'Attendance Rate'],
  ...
  const getReportSubtitle = () => {
    const category = filters.programCategory === 'all' ? 'All program categories' : filters.programCategory;
    const program = filters.program === 'all' ? 'All programs' : filters.program;
    const periodLabel = timePeriod.charAt(0).toUpperCase() + timePeriod.slice(1);
    return `${periodLabel} Report – ${category} – ${program}`;
  };

  const COLORS = ['#14B8A6', '#3B82F6', '#8B5CF6', '#F59E0B', '#10B981', '#EC4899'];
  ...
  return (
    <Layout title="View Reports">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 print:hidden">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Program category</label>
                <select
                ...
                >
                  ...
                  {PROGRAM_CATEGORIES.map(cat => (
                    <option
                      key={cat}
                      value={cat}
                      className={availableValues.programCategories.has(cat) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.programCategories.has(cat)}
                    >
                      {cat}
                    </option>
                  ))}
                ...
                >
                  ...
                  {programs.map(prog => (
                    <option
                      key={prog.id}
                      value={prog.name}
                      className={availableValues.programs.has(prog.name) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.programs.has(prog.name)}
                    >
                      {prog.name}
                    </option>
                  ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                >
                  ...
                  {AGE_GROUPS.map(age => (
                    <option
                      key={age}
                      value={age}
                      className={availableValues.ageGroups.has(age) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.ageGroups.has(age)}
                    >
                      {age}
                    </option>
                  ))}
                ...
                >
                  ...
                  {GENDERS.map(gen => (
                    <option
                      key={gen}
                      value={gen}
                      className={availableValues.genders.has(gen) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.genders.has(gen)}
                    >
                      {gen}
                    </option>
                  ))}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">ATSI status</label>
                <select
                  value={filters.atsiStatus}
                  onChange={(e) => handleFilterChange('atsiStatus', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                >
                  ...
                  {ATSI_STATUS.map(status => (
                    <option
                      key={status}
                      value={status}
                      className={availableValues.atsiStatuses.has(status) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.atsiStatuses.has(status)}
                    >
                      {status}
                    </option>
                  ))}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">CALD background</label>
                <select
                  value={filters.caldBackground}
                  onChange={(e) => handleFilterChange('caldBackground', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                >
                  ...
                  {CALD_BACKGROUND.map(bg => (
                    <option
                      key={bg}
                      value={bg}
                      className={availableValues.caldBackgrounds.has(bg) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.caldBackgrounds.has(bg)}
                    >
                      {bg}
                    </option>
                  ))}
                ...
                >
                  ...
                  {COUNCILS.map(council => (
                    <option
                      key={council}
                      value={council}
                      className={availableValues.councils.has(council) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.councils.has(council)}
                    >
                      {council}
                    </option>
                  ))}
                ...
                >
                  ...
                  {ADELAIDE_HILLS_TOWNSHIPS.map(town => (
                    <option
                      key={town}
                      value={town}
                      className={availableValues.townships.has(town) ? 'text-black' : 'text-gray-400'}
                      disabled={!availableValues.townships.has(town)}
                    >
                      {town}
                    </option>
                  ))}
              ...
              {showExportMenu && (
                <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                  <button
                    ...
                    className="block w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 rounded-t-lg whitespace-nowrap"
                  >
                    Export as PDF
                  </button>
                  <button
                    ...
                    className="block w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 rounded-b-lg whitespace-nowrap"
                  >
                    Export as CSV
                  </button>
                </div>
        ...
        {/* Report Preview */}
        {showPreview && (
          <div ref={reportRef} className="bg-white rounded-2xl shadow-lg p-8 mb-8">
            {/* Report Header */}
            <div className="mb-8 border-b-2 border-gray-200 pb-6">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  <img src={logo} alt="The Hut" className="h-16 w-auto" />
                  <div>
                    <h1 className="text-3xl font-bold text-gray-900">The Hut Community Centre</h1>
                    <p className="text-lg text-gray-600 mt-1">{getReportSubtitle()}</p>
                  </div>
                </div>
                <div className="text-right text-sm text-gray-600">
                  <div>From: {new Date(filters.startDate).toLocaleDateString()}</div>
                  <div>To: {new Date(filters.endDate).toLocaleDateString()}</div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              <div className="bg-gradient-to-br from-teal-500 to-teal-600 text-white rounded-xl p-6 shadow-lg">
                <div className="text-sm font-semibold mb-2">Unique Participants</div>
                <div className="text-4xl font-bold">{reportData.uniqueParticipants}</div>
              </div>
              <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl p-6 shadow-lg">
                <div className="text-sm font-semibold mb-2">Total Attendances</div>
                <div className="text-4xl font-bold">{reportData.totalAttendances}</div>
              </div>
              <div className="bg-gradient-to-br from-purple-500 to-purple-600 text-white rounded-xl p-6 shadow-lg">
                <div className="text-sm font-semibold mb-2">Total Records</div>
                <div className="text-4xl font-bold">{reportData.totalRecords}</div>
              </div>
              <div className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-xl p-6 shadow-lg">
                <div className="text-sm font-semibold mb-2">Attendance Rate</div>
                <div className="text-4xl font-bold">{reportData.attendanceRate.toFixed(1)}%</div>
              </div>
            </div>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        ...
                        dataKey="value"
                      >
                        {reportData.ageDistribution.map((entry, index) => (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-gray-200">
                      ...
                      <th className="text-right py-3 px-4 font-semibold text-gray-900">Unique Participants</th>
                      <th className="text-right py-3 px-4 font-semibold text-gray-900">Attendances</th>
                      <th className="text-right py-3 px-4 font-semibold text-gray-900">Attendance Rate</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reportData.programData.length > 0 ? (
                      reportData.programData.map((prog, idx) => (
                        <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                          <td className="py-3 px-4">{prog.program}</td>
                          <td className="py-3 px-4">{prog.category}</td>
                          <td className="text-right py-3 px-4">{prog.uniqueParticipants}</td>
                          <td className="text-right py-3 px-4">{prog.attendances}</td>
                          <td className="text-right py-3 px-4">{prog.attendanceRate.toFixed(1)}%</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
